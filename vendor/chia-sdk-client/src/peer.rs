...
use chia_protocol::{
    Bytes32, ChiaProtocolMessage, CoinStateFilters, Message, ProtocolMessageTypes,
...
impl Peer {
    ...
    /// Sends a fully-formed wire `Message`, preserving `id` for correlated RPC replies.
    ///
    /// Used when answering inbound `RequestPeers` (forwarded on the `mpsc` receiver from
    /// `from_websocket`) with `RespondPeers` carrying the same `id`.
    pub async fn send_protocol_message(&self, message: Message) -> Result<(), ClientError> {
    ...
    while let Some(message) = stream.next().await {
        ...
        match message {
            ...
            Binary(binary) => {
                ...
                // Remote `RequestPeers` uses ids from the sender's `RequestMap`. Our outbound
                // `request_raw` may use the *same* numeric id for an unrelated waiter; matching
                // `remove(id)` first would deliver `RequestPeers` to a `RespondPeers` waiter and
                // yield `ClientError::InvalidResponse`. Always treat inbound `RequestPeers` as
                // application-level RPC (see `send_protocol_message` for replies).
                if message.msg_type == ProtocolMessageTypes::RequestPeers {
                    sender.send(message).await.ok();
                    continue;
                }
