...
impl TlsAcceptor {
    pub fn new(builder: &TlsAcceptorBuilder) -> Result<TlsAcceptor, Error> {
        ...
        // dig-gossip vendor patch — CON-009 / Chia `server.py:67` (mTLS inbound).
        //
        // Default OpenSSL server verify does not require a client cert. Chia uses
        // `ssl.CERT_REQUIRED`. We trust the Chia CA (`chia_ca.crt` next to this crate’s manifest).
        acceptor.set_verify(SslVerifyMode::PEER | SslVerifyMode::FAIL_IF_NO_PEER_CERT);
        const CHIA_CA_PEM: &[u8] = include_bytes!(concat!(env!("CARGO_MANIFEST_DIR"), "/chia_ca.crt"));
        let chia_ca = X509::from_pem(CHIA_CA_PEM)?;
        acceptor.cert_store_mut().add_cert(chia_ca.clone())?;
        acceptor.add_client_ca(&chia_ca)?;
