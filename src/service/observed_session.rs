...
impl std::fmt::Debug for ObservedSession {
    /// `dig_nat::ClosedHandle` is not `Debug`, and the one fact worth printing about a session is
    /// whether it is still up — which is one atomic load.
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
