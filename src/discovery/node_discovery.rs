...
//!   DNS resolution MUST go through [`dig_peer_protocol::Network::lookup_all`] (normative
...
/// [`Network::lookup_all`](dig_peer_protocol::Network::lookup_all).
///
/// # Parameters
///
...
async fn try_introducer_query(
    ...
    let cert = match (&config.cert_path, &config.key_path) {
        (cert_p, key_p) if !cert_p.is_empty() && !key_p.is_empty() => {
            dig_peer_protocol::load_ssl_cert(cert_p, key_p)
                .map_err(|e| crate::error::GossipError::IntroducerError(format!("TLS load: {e}")))?
        }
        _ => dig_peer_protocol::ChiaCertificate::generate()
