...
///
/// SPEC §5.3 — certificate management via chia-ssl: load_ssl_cert() loads existing certificates.
/// SPEC §1.2 — chia-ssl crate dependency for TLS certificate generation and loading.
#[test]
...
///
/// SPEC §5.3 — ChiaCertificate::generate() creates new node certificates on first run.
#[test]
...
///
/// Proves SPEC §5.1 step 2 — create connector via create_native_tls_connector().
/// SPEC §5.3 — outbound mTLS: connector includes the node's own certificate (client cert).
#[test]
...
///
/// SPEC §5.1 step 4 — wrap in PeerConnection with gossip metadata from Handshake.
...
///
/// SPEC §2.4 — PeerConnection.creation_time: timestamp when connection was established (Unix seconds).
#[tokio::test]
...
/// Spin up a [`GossipService`] with TLS certs and return the temp dir (for lifetime),
/// the service (for stop), and the handle (for `connect_to`).
///
/// Used by: `test_outbound_connect_handshake`, `test_request_peers_after_connect`,
/// `test_outbound_connect_failure`.
...
///
/// Proves SPEC §5.1 step 5 — add peer to address manager.
/// SPEC §5.1 step 6 — send RequestPeers for discovery (node_discovery.py:135-136).
/// SPEC §6.6 — peer exchange: RespondPeers.peer_list is Vec<TimestampedPeerInfo>,
///   fed to address_manager.add_to_new_table().
#[tokio::test]
