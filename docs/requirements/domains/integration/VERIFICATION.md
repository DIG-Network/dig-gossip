...
| INT-008 | verified | Discovery loop spawned in start() | `tests/int_008_tests.rs`: start() spawns discovery loop task; DNS seed resolution → AddressManager merge; introducer backoff; serial connection attempts via `HandleDialer::dial` → `connect_via_nat_full_ladder` → `adopt_nat_connection` and `connect_to` → `connect_outbound_peer`; loop repeats on peer_connect_interval |
