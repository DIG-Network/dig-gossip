...
| DSC-009 | planned  | Parallel connection establishment           | Future roadmap; stub removed in v0.17.6. Current dialers: `HandleDialer::dial` → `connect_via_nat_full_ladder` → `adopt_nat_connection` and `connect_to` → `connect_outbound_peer` (CON-001). |
